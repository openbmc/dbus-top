                    ...
                    SensorSnapshot** sensor_snapshot)
{
    ...
    for (const auto& hwmon_entry : std::filesystem::directory_iterator(HWMON_ROOT)) {
            ...
            if (sp.size() == 3 &&
                ...
                if (ofnode2hwmon.find(sp[2]) != ofnode2hwmon.end()) {
                    ...
                    for (const std::string& op : objpaths)
                    {
                        ...
                        {
                            (*sensor_snapshot)->SetSensorVisibleFromHwmon(
                                service, op, hwmon_path);
                        }
                    }
                }
            }
